...
history = open(script_dir + '/history.txt', 'r+', encoding='utf-8')

bot = telebot.TeleBot('5927068331:AAGCQaUOJ4uBvUmRnM2_gQMzGOhNP39Jb_M')
...
buttons1.row(telebot.types.KeyboardButton('Вывести лог'))
...
def answer(msg: telebot.types.Message):
    ...
    if msg.text == 'Комплексные':
        ...
        message(chat_id=msg.from_user.id,
                text='Введите первое число.',
                reply_markup=del_buttons)
    elif msg.text == 'Вывести лог':
        bot.register_next_step_handler(msg, second_msg)
        bot.send_document(chat_id=msg.from_user.id, document=open("history.txt", "r"))
